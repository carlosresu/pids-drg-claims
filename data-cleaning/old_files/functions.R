...
read_entire_file <- function(drop_cols) {
  ...
  dt <- fread(full_claims_file, na.strings = na_values, drop = drop_cols)
  # toc()
  return(dt)
}
...
read_sampled_file <- function() {
  ...
  dt <- fread(sampled_claims_file, na.strings = na_values)
  return(dt)
}
...
process_icd10_mapping <- function(dt) {
  ...
  icds <- unique(c(
    unlist(strsplit(dt$PRIMARY_ILLNESS, ",")),
  ...
  cat(sprintf(
    "There are %d unique entries for ICD-10 codes, of which %d (%.2f%%) are already in the Thai ICD-10 library\n",
