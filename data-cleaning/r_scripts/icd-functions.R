...
remove_lumped_icd_codes <- function(column) {
  ...
  #'
  #' @details
  #' This function processes the specified column to remove lumped ICD
  #' codes by adding "||" between numeric and alphabetic characters.
...
transfer_extra_icd10s_to_clin_icd <- function(clin_icd, col) {
  ...
  #'
  #' @details
  #' This function processes the specified column by performing the
  #' following operations:
  #' - Ensures the column is a list of characters.
  #' - For rows with more than one element, splits and assigns the
  #' first element to the specified column.
...
# Helper function to extract unique ICD codes from specific columns
...
# Helper function to identify direct matches of ICD codes in the
# Thai ICD-10 library
...
# Helper function to map ICD-10 codes in specific columns
...
ensure_unique_icd_codes <- function(
  ...
  #' - Ensures that entries in clin_c1 are not found in
  #' clin_c2 or clin_icd within each row.
  #' - Ensures that entries in clin_c2 are not found in
  #' clin_c1 or clin_icd within each row.
  #' - Ensures that entries in clin_icd are not found in
  #' clin_c1 or clin_c2 within each row.
  ...
  unique_clin_icd <- mapply(function(c1, c2, icd) {
    setdiff(icd, union(c1, c2))
  }, clin_c1, clin_c2, clin_icd, SIMPLIFY = FALSE)

  # Ensure that clin_c1 and clin_c2 are unique within their columns
  unique_clin_c1 <- mapply(function(c1, c2, icd) {
    setdiff(c1, c2)
  }, clin_c1, clin_c2, SIMPLIFY = FALSE)
  unique_clin_c2 <- mapply(function(c1, c2, icd) {
    setdiff(c2, c1)
  }, clin_c1, clin_c2, SIMPLIFY = FALSE)

  return(
    list(
      clin_c1 = unique_clin_c1,
      clin_c2 = unique_clin_c2,
      clin_icd = unique_clin_icd
    )
  )
}
