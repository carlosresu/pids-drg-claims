---
title: An R Markdown document converted from "~/drg-pipeline/data-cleaning/drg-cleaning.ipynb"
...
### Read or Load Total Rows, Compute Sample Size, Assign Suffix
...
## Read, Process, Export Data (Call Unified Block)
...
```{r}
# Step 1: Source the necessary R scripts
source(here("data-cleaning", "py_scripts", "reticulate.R"))

# Copy the master_dt to avoid modifying the original data
python_input_dt <- data.table::copy(master_dt)
python_input_dt[, pat_bdate_orig := pat_bdate]
...
positive_age_dt[, pat_bdate := format(dmy(generate_dob(pat_bdate_orig, pat_age, date_adm)), "%m/%d/%Y")]

# For negative `pat_age`, keep `pat_bdate` as it is
negative_age_dt[, pat_bdate := format(mdy(pat_bdate_orig), "%m/%d/%Y")]

# Combine the processed data
result_dt <- rbind(positive_age_dt, negative_age_dt)

# Step 3: Import the necessary Python script
source_python(here("data-cleaning", "grouper", "seeker.py"))

# Step 4: Convert the `data.table` (master_dt) to a pandas DataFrame
py$pandas_df <- py$pd$DataFrame(result_dt)

# Step 5: Access any necessary Python libraries or classes
py$libs <- py$Libraries()

# Step 6: Subset the DataFrame in Python, process `clin_icd` and `icd9_list`, and replace `NaN` and `<NA>` with "None"
py_run_file(here("data-cleaning", "py_scripts", "convert_to_dict.py"))

# Step 7: Convert the subsetted DataFrame back to R
subset_df <- py$output

# Step 8: Generate the 'ageday' column based on 'pat_bdate'
# Initialize the 'ageday' column with "None"
subset_df$ageday <- "None"

# Filter rows where pat_age is a non-negative integer
valid_rows <- subset_df$pat_age >= 0 & subset_df$pat_age %% 1 == 0

# Calculate 'ageday' only for valid rows
subset_df$ageday[valid_rows & subset_df$pat_age == 0] <- as.numeric(difftime(mdy(subset_df$date_adm[valid_rows & subset_df$pat_age == 0]),
  mdy(subset_df$pat_bdate[valid_rows & subset_df$pat_age == 0]),
  units = "days"
))

# Step 9: Display the final DataFrame including the 'pat_bdate' and 'ageday' columns
print(head(subset_df, 25))
```
