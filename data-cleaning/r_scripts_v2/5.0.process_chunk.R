process_chunk <- function(chunk,
  ...
  # print(unique(nchar(unlist(chunk$clin_rvs))))
  # print(unique(unlist(chunk$clin_rvs)))

  # Count total elements in clin_rvs that are not 5 characters long
  not_five_char_count <- sum(sapply(chunk$clin_rvs, function(row) sum(nchar(row) != 5, na.rm = TRUE)))

  # Print result
  cat("Total number of elements in clin_rvs that are not 5 characters long:", not_five_char_count, "\n")
